if (KH_var_viewTargetCheckFrame isEqualTo diag_frameNo) exitWith {
    [
...
KH_var_viewTargetSurfaceDistance = _viewPosition vectorDistance (_viewTarget param [0, _viewPosition]);
...
KH_var_viewTargetDistance = if !(isNull KH_var_viewTargetObject) then {
    ...
    _viewPosition vectorDistance (_viewTarget param [0, _viewPosition]);
};
