class KH_ModuleAssignTerminal: Module_F
{
	...
	{
		class KH_ModuleAssignTerminalName: Edit
		{
			displayName = "Name";
		...
		class KH_ModuleAssignTerminalIdentifier: Edit
		{
		...
		class KH_ModuleAssignTerminalCondition: EditCodeMulti5
		{
			displayName = "Condition";
			tooltip = "Unscheduled code, executed locally to the player interacting with the terminal, that must return true in order for the terminal to be accessible for that player.";
			property = "KH_ModuleAssignTerminalCondition";
			defaultValue = "'true'";
		};
		class KH_ModuleAssignTerminalFunction: EditCodeMulti5
		{
			displayName = "Function";
			tooltip = "Unscheduled code, executed locally to the server, that must return a string that will be added on the terminal's console. If no string is returned, nothing is added to the output. Passed arguments available through _this are: [_input (STRING), _identifierOutput (STRING)]. _input is the text that a player inserted into the input field of the console. _identifierOutput dictates the text displayed on all terminals sharing the same identifier, and can be used for extra control through the KH_fnc_setTerminalText function which accepts the following arguments: [_identifierOutput (STRING), _text (STRING), _overwrite (BOOL)]. Line breaks in text can be added by including a backslash followed by the letter n. The terminal output itself can be accessed by external scripts as a missionNamespace variable 'KH_var_terminalOutput_%1', where %1 is the desired identifier.";
