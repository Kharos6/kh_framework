...
// slots at once, empty ones included, so a handler always hears the initial
// state. The compare is case-insensitive (the stored copy is lower-cased); the
// event carries the class name as the engine returned it. Slot names: PRIMARY
// = primaryWeapon, SECONDARY = handgunWeapon, TERTIARY = secondaryWeapon.
// The events are fired after the walk, from a list gathered during it: a
// handler may edit kh_var_allmen, and the walk reads that very array.
struct KhUnitWeapons {
    std::string slot[3];   // Lower-cased: primary, handgun, secondary.
...
static void update_unit_states() {
    ...
    for (auto it = g_unit_weapons.begin(); it != g_unit_weapons.end(); ) {
        if (!it->second.seen_this_frame) it = g_unit_weapons.erase(it);
        else ++it;
    }
...
// (RenderObject::sim_lod; the kernel is kh_simlod_share). On by default at
// the defaults; false or [] turns it off (the solver runs the object's own
// iterations / substeps whole wherever it is); true turns it on keeping the
// dials as they stand; [key, value] pairs turn it on and set those keys:
// floor (0.05..1, the least share; 1 reads as off), fullDistance and
// minDistance (m), fullScreenShare and minScreenShare (of the screen
// height). The pairs must order: fullDistance < minDistance, minScreenShare
// < fullScreenShare (checked on the resulting set, so either may be set
// alone while the other stands).
static bool kh_rv_sim_lod(const game_value& val, RenderIntegration::RenderObject& obj, std::string& err) {
    std::vector<std::pair<std::string, game_value>> khsl_p;
    bool khsl_on = false;
    if (!kh_rv_cloth_pairs(val, "simulationLod", khsl_p, khsl_on, err)) return false;
    float khsl_v[5];
    memcpy(khsl_v, obj.sim_lod, sizeof(khsl_v));
    for (const auto& khsl_kv : khsl_p) {
        const std::string& k = khsl_kv.first;
        const game_value& v = khsl_kv.second;
        if      (k == "floor")           { if (!kh_rv_cloth_num(v, "floor", 0.05f, 1.0f, khsl_v[0], err)) return false; }
        else if (k == "fulldistance")    { if (!kh_rv_cloth_num(v, "fullDistance", 0.0f, 1.0e5f, khsl_v[1], err)) return false; }
        else if (k == "mindistance")     { if (!kh_rv_cloth_num(v, "minDistance", 0.0f, 1.0e5f, khsl_v[2], err)) return false; }
        else if (k == "fullscreenshare") { if (!kh_rv_cloth_num(v, "fullScreenShare", 0.0f, 1.0f, khsl_v[3], err)) return false; }
        else if (k == "minscreenshare")  { if (!kh_rv_cloth_num(v, "minScreenShare", 0.0f, 1.0f, khsl_v[4], err)) return false; }
        else {
            err = "unknown simulationLod key '" + k + "' (floor | fullDistance | minDistance | fullScreenShare | minScreenShare)";
            return false;
        }
    }
    if (khsl_on && !(khsl_v[1] < khsl_v[2])) { err = "simulationLod: fullDistance must be less than minDistance"; return false; }
    if (khsl_on && !(khsl_v[4] < khsl_v[3])) { err = "simulationLod: minScreenShare must be less than fullScreenShare"; return false; }
    memcpy(obj.sim_lod, khsl_v, sizeof(obj.sim_lod));
    obj.sim_lod_on = khsl_on;
    return true;
}
...
static bool kh_rv_physics_collider(const game_value& val, RenderIntegration::RenderObject& obj, std::string& err) {
    ...
    bool khcc_convex = obj.physics_col_convex;   // KH_COL_CONVEX.
    for (size_t khcc_i = 0; khcc_i < khcc_p.size(); ++khcc_i) {
        ...
        else if (k == "convex")   { if (!kh_rv_bool(v, khcc_convex, "convex", err)) return false; }   // KH_COL_CONVEX.
        else {
            err = "unknown physics collider parameter '" + k + "' (margin | friction | inside | convex)";
            return false;
        }
    }
    ...
    obj.physics_col_convex = khcc_convex;   // KH_COL_CONVEX.
...
static bool kh_update_one(const char* cmd, bool want_fullscreen,
    ...
    // render thread blocked on this mutex). Sound because the write-back
    // merges: the fields other writers keep writing meanwhile (the attachment
    // pose - kh_attach_step runs on the render thread too - and the skinned /
...
// KH_DLS_ALLOW: allowDynamicShadows true / false. Global; off by default and
...
static game_value allow_dynamic_shadows_sqf(game_value_parameter arg) {
    try {
        ...
        RenderIntegration::g_dls_allowed.store(static_cast<bool>(arg), std::memory_order_relaxed);
        return game_value(true);
    } catch (...) {
        report_error("allowDynamicShadows: unknown exception");
        return game_value(false);
    }
}
...
static game_value get_render_stats_sqf() {
    try {
        ...
        float khd_cam[3] = {};
        // KH_DLSW_REF: the world pass's harvested references, read at the call
        // (no per-frame cost): the published lighting block (its denominator's
        // sky and sun), the dynamic-light reference (diffuse multiplier and
        // distance scale) and the shadow slots.
        float khd_blk[7] = {};                  // valid, sun rgb, ambient rgb.
        float khd_ref[5] = {};                  // valid, diffuse multiplier rgb, distance scale.
        static std::vector<float> khd_slots;    // live slots x 7: slot, published, light id, x, y, z, spot.
        khd_slots.clear();
        ...
        for (int attempt = 0; attempt < 4 && !khrt_got; ++attempt) {
            ...
            khd_blk[0] = RenderIntegration::g_pub_block_valid ? 1.0f : 0.0f;   // KH_DLSW_REF.
            for (int k = 0; k < 3; ++k) {
                khd_blk[1 + k] = RenderIntegration::g_pub_block_sun[k];
                khd_blk[4 + k] = RenderIntegration::g_pub_block_amb[k];
                khd_ref[1 + k] = khd.main_gdiff[k];
            }
            khd_ref[0] = khd.main_ref_valid ? 1.0f : 0.0f;
            khd_ref[4] = khd.main_scale;
            for (uint32_t s = 0; s < RenderIntegration::KH_DLS_MAX; ++s) {
                const RenderIntegration::KhDlsSlot& khd_sl = RenderIntegration::g_dls[s];
                if (!khd_sl.live) continue;
                khd_slots.push_back(static_cast<float>(s));
                khd_slots.push_back(RenderIntegration::kh_dls_published(s) ? 1.0f : 0.0f);
                khd_slots.push_back(static_cast<float>(khd_sl.light_id));
                khd_slots.push_back(khd_sl.pos[0]);
                khd_slots.push_back(khd_sl.pos[1]);
                khd_slots.push_back(khd_sl.pos[2]);
                khd_slots.push_back(static_cast<float>(khd_sl.spot));
            }
        ...
        out.push_back(kv("skinGpuOff", RenderIntegration::g_skin_so_failed.load(std::memory_order_relaxed) ? 1.0f : 0.0f));
        {
            ...
            float khsg_r = 0.0f;
            memcpy(&khsg_r, &khsg_b, sizeof(khsg_r));
        ...
        // KH_DL_DIAG (counted while collection is armed, zeroed by
        // resetRenderStats and the session reset): pool harvests by the
        // injection and by the flush's fallback (a cycle no injection
        // harvested); harvests known incomplete (the TTL sweep ran); author
        ...
        out.push_back(kv("dlWindowsRefused", static_cast<float>(s.dl_windows_refused)));
        ...
        {   // KH_DLSW_REF: see the snapshot above.
            auto_array<game_value> blk, ref, slots;
            for (float v : khd_blk) blk.push_back(game_value(v));
            for (float v : khd_ref) ref.push_back(game_value(v));
            for (size_t i = 0; i + 7 <= khd_slots.size(); i += 7) {
                auto_array<game_value> e;
                for (size_t f = 0; f < 7; ++f) e.push_back(game_value(khd_slots[i + f]));
                slots.push_back(game_value(std::move(e)));
            }
...
static void initialize_sqf_integration() {
    ...
    _sqf_allow_dynamic_shadows = intercept::client::host::register_sqf_command(
        "allowDynamicShadows",
        "true / false. Whether dynamic lights (flashlights, vehicle lights, lamps, muzzle flashes) cast shadows from our meshes - onto our meshes and onto the game world. Off by default and at every mission start; the lights still light our meshes either way. Global; applies from the next frame. Returns true on accept",
