...
static bool kh_apply_render3d_prop(RenderIntegration::RenderObject& obj, const std::string& handle,
    ...
    err = "unknown property (position | attachPosition | size | rotation | attachRotation | mesh | material | mode | sceneRead | effect | params | lit | twoSided | lodLock | inFront | casterOnly | clothSimulation | physicsCollider | chainSimulation | color | visible | blend | band | duration)";
    return false;
}
