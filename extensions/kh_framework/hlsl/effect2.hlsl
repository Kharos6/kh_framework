...
R"HLSL(    else if (effect == 22)   // ssgi: [intensity, radiusM, samples 4..32, normalBias] + [falloffPow, saturation, maxDistM (0 = off),
    {
        ...
        // scene/camera change; - the z-fight / acne class (self-sampling
        // through quantized depth) is excluded twice: a 2 px minimum tap
        // offset, and the normalBias floor on the cosine lobe; - normals come
        // from the smaller-delta side per axis (the silhouette-safe
        // reconstruction - a naive derivative cross paints edge pixels with
        // cross-object normals); - HDR fireflies (bright speculars
        // entering/leaving the tap set under camera motion) are capped by
        // lumClamp; - normalization is by TAP COUNT, never by surviving
        // weight: a sparse valid set DIMS instead of amplifying lone taps
        // (weight-sum division turns one surviving bright tap into a
        // full-strength sparkling pixel).
        ...
        if (khg_cd < 1e8f && khg_rng > 0.001f && depthParams.y < -1.0e-3f)
        {
            ...
            // Fix: reconstruct in VIEW space (KhgVpos: pixel + linear depth
            // through the recovered projection diagonal), which is
            // camera-relative by construction - magnitudes never exceed the
            // far plane, so the cancellation cannot occur.
            ...
            if (khg_nl > 1e-9f)
            {
                ...
                // (0.02..0.10 -> 0.008..0.05) - the bilateral resolve now
                // eats the reconstruction noise this fade guarded against, so
                ...
                // The cap existed for variance; variance is now owned by the
                // MIP TAPS below (each tap samples the mip matching its
                // annulus footprint = pre-filtered radiance, flat variance at
                // any spread) plus the doubled a-trous + resolve. 2048 is a
                // sanity ceiling, not a design point.
...
R"HLSL(
                ...
                // lost its DOMINANT nearby taps - per-pixel admission
                // variance, exactly the gap texture.
                ...
                [loop] for (int khg_k = 0; khg_k < khg_n; ++khg_k)
                {
                    ...
                    // A golden-ratio offset PER STRATUM decorrelates the
                    // bands: still position-only (seedless doctrine), still
                    // one tap per annulus; the rings collapse into isotropic
                    // variance the smoothing chain already eats.
                    float khg_igk = frac(khg_ig2 + khg_kp * 0.61803399f);
                    float khg_sr = max(sqrt((khg_kp + khg_igk) / khg_hn) * khg_spx, 2.05f);   // jitter; floor clamp; per-stratum
                    ...
                    // reconstruct FAR in world and die here smoothly - the
                    // halo class needs no depth-compare verdict
...
R"HLSL(
                    ...
                    // The leak class is SHALLOWER-than-receiver samples at
                    // near-perpendicular emission (dot slightly negative,
                    // living off the slack floor); genuine lateral transport
                    // from DEEPER samples carries a positive dot and never
                    // needed it.
                    ...
                    float khg_w = khg_ndl * khg_cs * khg_pw * khg_occ * pow(saturate(1.0f - khg_d / khg_rad), khg_fp);   // + khg_pw; + khg_occ
...
R"HLSL(
                ...
                // Off-screen taps are ABSENT INFORMATION, not absent
                // geometry, yet the fixed 1/n normalization dimmed edge
                // receivers by exactly the lost fraction. Normalize by the
                // on-screen count instead, floored at n/2 so the anti-sparkle
                // direction survives (geometric rejection still dims); an
                // edge receiver recovers at most 2x.
                ...
                // receiver-albedo proxy: bounce lands tinted by the surface
                // it lights (scene chroma over a luma floor) reload at FULL
                // res - the head's 'scene' sampled the half grid's
                // coordinates (the top- left quadrant), a wrong-texel tint on
                // every receiver.
        ...
        if (khr_cd < 1e8f)
        {
            ...
            [unroll] for (int khr_i = -2; khr_i <= 2; ++khr_i)
            {
                ...
                float khr_w = exp(-0.125f * (khr_i * khr_i + khr_j * khr_j))
                            * exp(-khr_dz * khr_dz);
                // gather is half-res - full-res depth guiding half-res
                // radiance = joint bilateral UPSAMPLE. Normalized coords are
...
R"HLSL(    else if (effect == 25)
    ...
    {
        // RADIANCE PYRAMID SEED (internal id, the group's first draw:
        // setPostFX validates <= KH_MAX_EFFECT = 23, so 24/25/26 stay
        // unreachable from SQF; the flush synthesizes this ahead of the
        // gather). i.pos spans the SCALED grid, so the full-frame uv rebuilds
