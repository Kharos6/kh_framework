...
static game_value set_render_debug_sqf(game_value_parameter arg) {
    try {
        ...
        // mesh to the union. The A/B for that retirement.
                            ...
                            khd_m == 584 ||   // KH_LIGHT_RECON OFF (26880, the REVERT): stands down the lighting reconciliation (once a second at 26880, every 250 ms since 26883). Default since 26880 publishes the MEDIAN of the last 1.5 s of raw cascade axes / lit-block candidates whenever the published sun direction (> 1 deg) or block (outside the gates' own 50% band) disagrees with it, and counts the move as settled - so no gate (jump cooldown, candidacy holds, sky-static refusal, cold hold, block pending / collapse / dark holds) can keep a skipTime or a 0 setOvercast from landing for more than ~1 s. Since 26881 the block ring is fed from the RAW upload at the probe, ahead of the probe's own collapse / starved / lit-floor holds (which had frozen the mirror every consumer reads, so 26880 could only agree with a value the engine had stopped sending), a reconciled block resets the probe's standing and the injection snapshot with it, and a cold hold whose direction the window already agrees with is closed (reconSunSettles). Under 584 the 26879 arbitration runs alone. The latency lanes (sunPubLatMs / sunSettleLatMs / blkLatMs and their *By / *Max) keep measuring under 584, which is how the two are compared.
                            khd_m == 585 ||   // KH_RECON_SUN_WITNESS OFF (26884, the REVERT): the lighting reconciliation publishes a sun consensus without asking whether it explains the sky CB's own sun lane at least as well as the standing publish does. Default since 26884 refuses a consensus that explains it worse (reconSunWitRefused) - the look-away/look-back blink of dump2, where camera turns re-laid the cascades and the median of that degraded stream was published, rebuilding the maps twice per look-back. Expect the double flicker and the self-shadow offset to return under 585.
                            khd_m == 586 ||   // KH_DL_LIST_CLAIM OFF (26884, the REVERT; widened at 26885): the dynamic-light pool goes back to matching a re-sighted light by position alone (a 1 m cube, first slot in range), the shadow-slot HOLD and lookup to the first slot in range (KH_DLS_HOLD_NEAREST: two lights inside a metre swapped slots on pool/slot order and rebuilt both maps, one shadow at a time), and - 26885 - the map render's spot-source pick back to the first pool entry in range with no spot-flag test (KH_DLS_SPOT_SRC: the flashlight's frustum was built from the feet light's POINT record, a 0.12 rad pencil, so the flashlight cast nothing while the feet light was lit, and a muzzle flash in the same metre flipped the pick). The pool light ids (KH_DL_POOL_ID) are ignored at every slot site under 586. Default since 26884 lets each record of ONE uploaded list claim a slot no earlier record of that list has claimed, nearest record winning - two engine lights inside a metre of each other were folding into one pool slot and overwriting it in turn (the fight). Expect co-located lights to alternate again under 586, and the flashlight+feet-light pair to lose one shadow; dlPoolSplits and dlsSpotSrc* read 0 here.
                            khd_m == 587 ||   // KH_RECON_BLK_COEXIST OFF (26884, the REVERT): the lighting reconciliation publishes a dimmer block consensus even while the window still carries the standing level as a live flavour. Default since 26884 refuses it (reconBlkCoexist) - a 1000x mesh's shadow covered most of the scene, the engine darkened the lit block of every object it drew inside it, that flavour became the window's majority and the median published it: the meshes dimmed to half while 30% of uploads were still bright. Expect that dim to return under 587; reconBlkOldFrac reads the bright share.
                            khd_m == 588 ||   // KH_BLK_PROP_DIM OFF (26884, the REVERT): a dimmer lit-block candidate whose sun/ambient RATIO matches the standing's is treated as a real level again, by the reconcile and by the probe's ten-second starved concede. Default since 26884 refuses it (reconBlkProp / blkPropHolds): a block scaled uniformly on both lanes is an object drawn inside a shadow, not the weather - the giant-shadow walk (x0.49) and the overcast-to-sun switch (x0.46, then x0.73) both dimmed our meshes through it while the world stayed bright. A real overcast moves the ratio by three orders and lands as before. Expect the post-switch dim to return under 588.
...
static game_value get_render_stats_sqf() {
    try {
        ...
        // KH_LIGHT_RECON (26880). reconChecks: once-a-second passes;
        // reconSun/BlkAdopts: passes that overrode a holding gate; *Short:
        // verdicts refused for too few window samples (a dead stream reads
        // here, not as a stale publish). Latency lanes measure the OTHER
        // path too: sunPubLatMs = first raw cascade sample > 1 deg off the
        // published direction -> the publish that closed it; sunSettleLatMs
        // = that publish -> kh_sun_settled (the cold hold / settle cost);
        // blkLatMs = first out-of-band block candidate -> adoption. *By: 1 =
        // the gates adopted, 2 = reconciliation did. Max resets with the
        // window. Under mode 584 these read the un-reconciled arbitration.
        out.push_back(kv("reconChecks", RenderIntegration::g_rc_checks));
        out.push_back(kv("reconSunAdopts", RenderIntegration::g_rc_sun_adopts));
        out.push_back(kv("reconBlkAdopts", RenderIntegration::g_rc_blk_adopts));
        out.push_back(kv("reconSunShort", RenderIntegration::g_rc_sun_short));
        out.push_back(kv("reconBlkShort", RenderIntegration::g_rc_blk_short));
        // 26881: cold holds closed because the published direction already
        // matched the window's consensus (the maps follow a gate-adopted
        // jump in <= 1 s instead of after the 1.25 s hold).
        out.push_back(kv("reconSunSettles", RenderIntegration::g_rc_sun_settles));
        // 26884: sun consensus verdicts refused by the sky-CB sun witness
        // (a foreign-majority window, not a move) / adopted with no usable
        // witness (fail-open, the gates' rule). Expect reconSunWitRefused to
        // climb on every look-away and reconSunAdopts to stay flat.
        out.push_back(kv("reconSunWitRefused", RenderIntegration::g_rc_sun_wit_refused));
        out.push_back(kv("reconSunWitOpen", RenderIntegration::g_rc_sun_wit_open));
        // 26884: pool entries created for a light whose 1 m cube already held
        // a slot claimed by an earlier record of the SAME list - the sightings
        // the old positional match folded into one slot (KH_DL_LIST_CLAIM).
        out.push_back(kv("dlPoolSplits", RenderIntegration::g_dl_pool_splits));
        // 26884: HLSL RCDATA resources kh_shaders.rc did not supply (0 = the
        // DLL carries every unit; non-zero = a unit is compiling from a
        // comment-only stand-in and its shaders cannot build).
        out.push_back(kv("hlslResMissing", static_cast<uint64_t>(RenderIntegration::g_hlsl_res_missing)));
        // 26884: dimmer block consensuses refused because the standing level was
        // still a live flavour in the window (KH_RECON_BLK_COEXIST), and the share
        // of the last verdict's window that sat inside the band around the
        // published sun (-1 = no verdict yet).
        out.push_back(kv("reconBlkCoexist", RenderIntegration::g_rc_blk_coexist));
        out.push_back(kvf("reconBlkOldFrac", RenderIntegration::g_rc_blk_old_frac));
        // 26884: dimmer candidates refused because they were a uniform scale of
        // the standing (sun/ambient ratio preserved, KH_BLK_PROP_DIM) - by the
        // reconcile, and by the probe ahead of its ten-second starved concede.
        out.push_back(kv("reconBlkProp", RenderIntegration::g_rc_blk_prop));
        out.push_back(kv("blkPropHolds", RenderIntegration::g_blk_prop_holds));
        // 26882: how far back the last verdict's median actually reached (the
        // ring is the window; the lit block arrives per draw, so this is
        // frames, not the 1.5 s cap).
        out.push_back(kv("reconSunSpanMs", RenderIntegration::g_rc_sun_span_ms));
        out.push_back(kv("reconBlkSpanMs", RenderIntegration::g_rc_blk_span_ms));
        out.push_back(kv("sunPubLatMs", RenderIntegration::g_rc_sun_pub_lat_ms));
        out.push_back(kv("sunPubLatMaxMs", RenderIntegration::g_rc_sun_pub_lat_max_ms));
        out.push_back(kv("sunPubLatBy", RenderIntegration::g_rc_sun_lat_by));
        out.push_back(kv("sunSettleLatMs", RenderIntegration::g_rc_sun_settle_lat_ms));
        out.push_back(kv("sunSettleLatMaxMs", RenderIntegration::g_rc_sun_settle_lat_max_ms));
        out.push_back(kv("blkLatMs", RenderIntegration::g_rc_blk_lat_ms));
        out.push_back(kv("blkLatMaxMs", RenderIntegration::g_rc_blk_lat_max_ms));
        out.push_back(kv("blkLatBy", RenderIntegration::g_rc_blk_lat_by));
    ...
    out.push_back(kvf("sunPfCpuMu3O", RenderIntegration::g_sun_pf_cpu[3]));
        ...
        out.push_back(kvf("dlAnchorGhostDz", RenderIntegration::g_dl.anchor_ghost_dz));
        {
            ...
            // nothing guards the constant at compile time any more; this guards the runtime.
            ...
            // KH_DLS_SPOT_SRC (26885). How the map render found each spot
            // slot's pool light. dlsSpotSrcId = by KH_DL_POOL_ID (the normal
            // case once a slot has held its light for one pass); dlsSpotSrcPos
            // = by the nearest-spot fallback (a slot admitted before an id
            // landed). dlsSpotSrcWrong counts fallbacks where the OLD
            // first-in-range scan would have handed the spot a POINT record -
            // the 26884 fault that built the flashlight's frustum from the
            // feet light. Non-zero here with a stable picture is the fix
            // working; non-zero under 586 is unreachable (586 keeps the old
            // scan, so nothing is counted).
            out.push_back(kv("dlsSpotSrcId", RenderIntegration::g_dls_spot_src_id));
            out.push_back(kv("dlsSpotSrcPos", RenderIntegration::g_dls_spot_src_pos));
            out.push_back(kv("dlsSpotSrcWrong", RenderIntegration::g_dls_spot_src_wrong));
